...
const TeacherAssignments = () => {
        ...
        for (const assignment of assignments) {
          try {
            ...
            const submissions = list.map(s => ({
              ...
              // Giữ nguyên submittedAt từ backend, format khi hiển thị
  ...
  const isAssignmentOngoing = (assignment) => {
    ...
    // Đối với bài thi, kiểm tra endTime
    if (assignment.isExam && assignment.endTime) {
      return new Date(assignment.endTime) >= now;
    }
                      ...
                      <PieChart>
                        <Pie
                          data={(() => {
                            ...
                            // Tính tổng sinh viên theo lớp, tránh cộng trùng
                            let totalStudents = 0;
                            if (courseFilter !== 'all') {
                              const classAssignments = filteredAssignments.filter(a => a.classId === courseFilter);
                              totalStudents = classAssignments.length > 0 ? (classAssignments[0].totalStudents || 0) : 0;
                            } else {
                              totalStudents = filteredAssignments.reduce(
                                (max, a) => Math.max(max, a.totalStudents || 0),
                                0
                              );
                            }
                            
                            const totalSubmitted = filteredAssignments.reduce((sum, a) => sum + a.submittedStudents, 0);
                            const totalNotSubmitted = Math.max(0, totalStudents - totalSubmitted);
                ...
                <Paper sx={{ p: 2 }}>
                  ...
                  <Box sx={{ p: 2 }}>
                    {(() => {
                      ...
                      let totalStudents = 0;
                      if (courseFilter !== 'all') {
                        // Đang lọc 1 lớp cụ thể → lấy số sinh viên của lớp đó
                        const classAssignments = filteredAssignments.filter(a => a.classId === courseFilter);
                        totalStudents = classAssignments.length > 0 ? (classAssignments[0].totalStudents || 0) : 0;
                      } else {
                        // Lọc tất cả lớp → lấy max số sinh viên trong các bài (tránh cộng trùng)
                        totalStudents = filteredAssignments.reduce(
                          (max, a) => Math.max(max, a.totalStudents || 0),
                          0
                        );
