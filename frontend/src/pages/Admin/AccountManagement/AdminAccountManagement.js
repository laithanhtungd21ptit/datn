...
const AdminAccountManagement = () => {
  ...
  const handleResetPasswordConfirm = async () => {
    ...
    try {
      await api.adminUpdateAccount(selectedUser.id, { resetPassword: true });
      openResultDialog('Thành công', `Đã reset mật khẩu cho tài khoản "${selectedUser.fullName}". Mật khẩu mới là "123456".`, 'success');
      ...
      openResultDialog('Lỗi', error?.message || 'Không thể reset mật khẩu. Vui lòng thử lại.', 'error');
    }
  };
  ...
  return (
    <Box>
      ...
      >
        ...
        <MenuItem onClick={() => handleResetPassword(selectedUserForMenu)}>
          <Security sx={{ mr: 1 }} />
          Reset mật khẩu
        </MenuItem>
        <Divider />
      ...
      >
        ...
        <DialogContent>
          <Alert severity="warning" sx={{ mb: 2 }}>
            Bạn có chắc chắn muốn reset mật khẩu cho tài khoản <strong>"{selectedUser?.fullName}"</strong>?
          </Alert>
          <Typography variant="body2" color="text.secondary">
            Mật khẩu mới sẽ được đặt là: <strong>"123456"</strong>
          </Typography>
          <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
            Người dùng sẽ cần đăng nhập lại với mật khẩu mới này.
          </Typography>
        </DialogContent>
        <DialogActions>
