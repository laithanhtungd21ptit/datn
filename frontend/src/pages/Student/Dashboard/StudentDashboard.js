  ...
  const data = await api.studentDashboard();
        ...
        if (Array.isArray(data?.upcomingExams)) {
          setUpcomingExams(data.upcomingExams.map((exam, idx) => ({
            ...
            requireMonitoring: exam.requireMonitoring || false,  
            ...
            <List sx={{ flex: 1 }}>
              {upcomingExams.slice(0, showAllExams ? upcomingExams.length : 4).map((exam, index) => (
                <ListItem key={index} divider button onClick={() => handleViewExam(exam)}>
                  ...
                  <ListItemText
                    primary={`${formatDateTime(exam.startAt)} - ${exam.title}`}
                    secondary={`${exam.class} | Thời lượng: ${exam.duration} phút | Giám sát: ${exam.requireMonitoring ? 'Có' : 'Không'}`}                  />
                </ListItem>
              ))}
        ...
        <DialogContent>
          ...
          <Grid container spacing={2} sx={{ mb: 3 }}>
            <Grid item xs={12} sm={6}>
              ...
              <Typography variant="body2">
                {formatDateTime(selectedExam?.startAt)}
              </Typography>
            </Grid>
            <Grid item xs={12} sm={6}>
              <Typography variant="subtitle2" color="text.secondary">
            ...
            <Grid item xs={12}>
              ...
              <Chip
                label={selectedExam?.requireMonitoring ? 'Bắt buộc bật camera' : 'Không yêu cầu giám sát'}
                color={selectedExam?.requireMonitoring ? 'warning' : 'default'}
                size="small"
              />
            </Grid>
          </Grid>
