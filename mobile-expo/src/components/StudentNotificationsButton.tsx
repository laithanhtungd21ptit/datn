...
const StudentNotificationsButton: React.FC = () => {
  ...
  const handleSelect = async (item: StudentNotification) => {
    setSelected(item);
    if (!item.isRead) {
      try {
        await api.studentMarkNotificationRead(item.id);
...
export default StudentNotificationsButton;
