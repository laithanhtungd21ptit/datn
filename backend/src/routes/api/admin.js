...
adminRouter.put('/accounts/:id', async (req, res) => {
  const { id } = req.params;
  const { fullName, email, role, status, password, phone, department, resetPassword } = req.body || {};
  
  // Get user info before update (for email notification)
  const userBeforeUpdate = await UserModel.findById(id).lean();
  if (!userBeforeUpdate) return res.status(404).json({ error: 'NOT_FOUND' });
  ...
  let shouldSendResetEmail = false;
  if (password) {
  ...
  if (shouldSendResetEmail && userBeforeUpdate.email) {
    try {
      await sendAdminPasswordResetEmail(
        userBeforeUpdate.email,
        userBeforeUpdate.fullName || userBeforeUpdate.username || 'bạn',
        '123456'
      );
      console.log(`Password reset email sent to ${userBeforeUpdate.email}`);
    } catch (error) {
...
adminRouter.post('/send-notification', async (req, res) => {
  ...
  try {
    ...
    try {
      const { emitNotificationToUser } = await import('../../services/socketService.js');
      const senderInfo = await UserModel.findById(req.user?.id).select('fullName').lean();
