...
studentRouter.get('/assignments', async (req, res) => {
  ...
  // Get class information for each assignment with teacher details
  const classMap = {};
  const classes = await ClassModel.find({ _id: { $in: classIds } }).populate('teacherId', 'fullName').lean();
  classes.forEach(cls => {
    classMap[cls._id.toString()] = cls;
...
studentRouter.get('/exams/:id', async (req, res) => {
  ...
  const submission = await SubmissionModel.findOne({ assignmentId: id, studentId }).lean();
...
studentRouter.post('/submissions', upload.array('files', 5), async (req, res) => {
  ...
  if (assignment.isExam) {
    // Với bài thi, cho phép nộp trong khoảng thời gian thi
    const startTime = assignment.startTime || assignment.dueDate;
    const duration = assignment.durationMinutes || 0;
    const endTime =
      assignment.endTime ||
      (duration ? new Date(new Date(startTime).getTime() + duration * 60000) : startTime);

    if (now < new Date(startTime) || now > new Date(endTime)) {
      return res.status(400).json({ error: 'EXAM_NOT_IN_PROGRESS' });
    }
  } else {
    // Với bài tập thường, không cho nộp sau hạn
  ...
  if (assignment.isExam) {
    try {
      ...
      console.log(`✅ Marked exam sessions as submitted for student ${studentId}, assignment ${assignmentId}`);
    } catch (error) {
      console.error('Error marking exam session as submitted:', error);
      // Không throw error vì submission đã thành công
    }
  }
