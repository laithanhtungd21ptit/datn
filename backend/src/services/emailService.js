...
const templates = {
  ...
  adminPasswordReset: ({ fullName, newPassword = '123456' }) => ({
    subject: 'Mật khẩu đã được reset - Assignment Management System',
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #0084FF;">Xin chào ${fullName},</h2>
        <p>Mật khẩu tài khoản của bạn đã được quản trị viên reset.</p>
        <div style="background-color: #f5f5f5; padding: 15px; border-radius: 5px; margin: 20px 0;">
          <p style="margin: 0;"><strong>Mật khẩu mới của bạn là:</strong></p>
          <p style="font-size: 24px; font-weight: bold; color: #0084FF; margin: 10px 0 0 0;">${newPassword}</p>
        </div>
        <p><strong>Vui lòng:</strong></p>
        <ul>
          <li>Đăng nhập lại với mật khẩu mới này</li>
          <li>Thay đổi mật khẩu sau khi đăng nhập để bảo mật tài khoản</li>
        </ul>
        <p style="color: #d32f2f;"><strong>Lưu ý:</strong> Nếu bạn không yêu cầu reset mật khẩu, vui lòng liên hệ với quản trị viên ngay lập tức.</p>
        <hr>
        <p style="color: #666; font-size: 12px;">Email này được gửi tự động từ Assignment Management System.</p>
      </div>
    `,
...
export const sendAdminPasswordResetEmail = (email, fullName, newPassword = '123456') => {
  return sendEmail(email, 'adminPasswordReset', { fullName, newPassword });
};
